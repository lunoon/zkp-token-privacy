...
import './IUnconfidentialRegistry.sol';

contract UnconfidentialRegistry is IUnconfidentialRegistry {

  function create(uint256 _balance) public override returns (bytes32) {
    // bytes32 fp = keccak256(abi.encode(_h0, _h1)) create fp in backend ffs
    // salt is added as random r to _balance similar to Socialist millionaire problem
    // ethers.utils.solidityKeccak256(["uint256"],[_balance])
    bytes32 fp = keccak256(abi.encode(_balance));
  ...
  function join(bytes32 fp0, bytes32 fp1, uint256 sumBalance, uint256 balance0, uint256 balance1) public override {
    require(entries[fp0] == msg.sender, "UF-J-01");
    require(entries[fp1] == msg.sender, "UF-J-02");
    require(fp0 != fp1, "UF-J-03");

    require(fp0 == keccak256(abi.encode(balance0)), "UF-J-04");
    require(fp1 == keccak256(abi.encode(balance1)), "UF-J-05");
    ...
    bytes32 sumFp = keccak256(abi.encode(sumBalance));
  ...
  function split(bytes32 sumFp, uint256 sumBalance, uint256 balance0, uint256 balance1) public override {
    require(entries[sumFp] == msg.sender, "UF-S-01");

    require(sumFp == keccak256(abi.encode(sumBalance)), "UF-S-02");
    ...
    bytes32 fp0 = keccak256(abi.encode(balance0));
    bytes32 fp1 = keccak256(abi.encode(balance1));
