...
initialize().then((zokratesProvider) => {
  ...
  fs.writeFile(path.resolve(__dirname, '../out/splitjoinProgram'), splitjoinProgram, (err) => console.log(err));
  ...
  fs.writeFile(path.resolve(__dirname, '../out/splitjoinKeypair'), objJsonB64, 'utf8', (err) => console.log(err));

  const verifier = zokratesProvider.exportSolidityVerifier(keypair.vk);
  fs.writeFile(path.resolve(__dirname, '../out/Verifier.sol'), verifier, (err) => console.log(err));
  // read with
